...
export const Route = createFileRoute("/admin/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Configure trigger rules, document & email templates, and role access." },
    ],
  }),
  component: AdminHome,
});
...
const sections = [
  { to: "/admin/triggers", icon: Settings, label: "Trigger rules", desc: "PIP & CAP automation thresholds, NI ratings, AES windows." },
...
function AdminHome() {
  return (
    <div>
      <PageHeader eyebrow="Admin" title="System configuration" description="Tune the engine driving consequence management." />
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2">
        {sections.map((s) => (
          <SectionCard key={s.to} title={s.label} description={s.desc} actions={<Button size="sm" variant="outline" asChild><Link to={s.to}>Open <ArrowRight className="ml-1 h-3 w-3" /></Link></Button>}>
