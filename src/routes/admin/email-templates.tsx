...
export const Route = createFileRoute("/admin/email-templates")({
  head: () => ({ meta: [{ title: "Admin — Email templates" }] }),
...
const emails = [
  ...
  { id: "pip-extension", name: "PIP extension — manager approval", to: "Manager", cc: "Supervisor" },
  ...
  { id: "cap-exception", name: "CAP exception — approver", to: "QA / Compliance Mgr / SDL", cc: "Supervisor" },
...
function EmailTemplates() {
  return (
    <div>
      <PageHeader eyebrow="Admin · Config" title="Email templates" description="System-generated emails across PIP & CAP lifecycle." />
          ...
          {emails.map((e) => (
            <li key={e.id} className="flex items-center justify-between gap-3 py-3 first:pt-0 last:pb-0">
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary-soft text-primary"><Mail className="h-4 w-4" /></div>
                <div>
                  <div className="text-sm font-semibold">{e.name}</div>
                  <div className="text-xs text-muted-foreground">To: {e.to} · CC: {e.cc}</div>
                </div>
              </div>
