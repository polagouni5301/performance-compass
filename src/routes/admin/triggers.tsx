...
export const Route = createFileRoute("/admin/triggers")({
  head: () => ({ meta: [{ title: "Admin — Trigger rules" }] }),
  component: Triggers,
});

const rules = [
  { name: "PIP · 3 consecutive warnings", window: "3 weeks", source: "Auto Scheduler", active: true },
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Button><Plus className="mr-2 h-4 w-4" /> Add rule</Button>}
      />
            ...
            {rules.map((r) => (
              <tr key={r.name} className="hover:bg-secondary/40">
                ...
                <td className="py-3 pr-3">
                  {r.active ? <StatusBadge variant="success">Active</StatusBadge> : <StatusBadge>Inactive</StatusBadge>}
                </td>
