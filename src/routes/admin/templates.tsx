...
export const Route = createFileRoute("/admin/templates")({
  head: () => ({ meta: [{ title: "Admin — Document templates" }] }),
  component: Templates,
});
...
function Templates() {
  return (
    <div>
      <PageHeader eyebrow="Admin · Config" title="Document templates" description="Versioned Word templates used during PIP & CAP generation." />
          ...
          <SectionCard key={t.name}>
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground">{t.role} · updated {t.updated}</div>
              </div>
            </div>
            <div className="mt-3 flex gap-2">
              <Button size="sm" variant="outline" className="flex-1"><Download className="mr-1 h-3.5 w-3.5" /> Download</Button>
              <Button size="sm" className="flex-1"><Upload className="mr-1 h-3.5 w-3.5" /> Upload v+</Button>
            </div>
          </SectionCard>
        ))}
      </div>
