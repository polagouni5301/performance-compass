...
export const Route = createFileRoute("/supervisor/pip/closure")({
  head: () => ({
    meta: [
      { title: "Supervisor — PIP Closure / Extension" },
      { name: "description", content: "Decide PIP outcome after the 4th review — close successfully or request 30-day extension." },
    ],
  }),
  component: Closure,
});
...
function Closure() {
  const cases = pipCases.filter((c) => c.reviews.length >= 4 && (c.status === "active" || c.status === "pending-approval"));
