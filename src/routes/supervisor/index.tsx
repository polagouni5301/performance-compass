...
export const Route = createFileRoute("/supervisor/")({
  head: () => ({
    meta: [
      { title: "Supervisor home — PIP & CAP" },
      { name: "description", content: "Supervisor cockpit for PIP candidate review, active reviews, and CAP accept/dispute." },
    ],
  }),
...
function SupervisorHome() {
  ...
  const upcoming = pipCases.flatMap((c) => c.reviews.filter((r) => r.status === "scheduled").map((r) => ({ c, r }))).slice(0, 4);
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline" asChild><Link to="/supervisor/pip/candidates"><Users className="mr-2 h-4 w-4" /> Candidates</Link></Button>
            <Button asChild><Link to="/supervisor/cap/inbox"><Inbox className="mr-2 h-4 w-4" /> CAP inbox</Link></Button>
          </>
        }
      />

      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <StatCard label="Candidates to confirm" value={candidatesPending} icon={Users} tone="warning" />
        <StatCard label="Active PIPs" value={pipCases.filter(c => c.status === "active").length} icon={TrendingUp} tone="primary" />
        <StatCard label="Reviews scheduled" value={upcoming.length} icon={CalendarCheck} />
        <StatCard label="CAPs awaiting action" value={capInbox.length} icon={ShieldAlert} tone="danger" />
      </div>

      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <SectionCard
          title="Upcoming 15-day reviews"
          actions={<Button size="sm" variant="ghost" asChild><Link to="/supervisor/pip/review">Open review cycle <ArrowRight className="ml-1 h-3 w-3" /></Link></Button>}
        >
          {upcoming.length === 0 ? (
              ...
              {upcoming.map(({ c, r }) => (
                <li key={c.id + r.number} className="flex items-center justify-between gap-3 py-3 first:pt-0 last:pb-0">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">{c.id} · Review {r.number} · {r.date}</div>
                  </div>
        ...
        <SectionCard
          ...
          actions={<Button size="sm" variant="ghost" asChild><Link to="/supervisor/cap/inbox">Open <ArrowRight className="ml-1 h-3 w-3" /></Link></Button>}
        >
          {capInbox.length === 0 ? (
