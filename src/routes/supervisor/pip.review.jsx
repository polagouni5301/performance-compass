                  ...
                  <li key={i} className="rounded-xl border border-border bg-card p-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <div className="flex gap-2">
                        <Button
                          size="sm"
                          variant={choice === "met" ? "default" : "outline"}
                          onClick={() => setDecisions({ ...decisions, [key]: "met" })}
                        >
                          <Check className="mr-1 h-3.5 w-3.5" /> Met
                        </Button>
                        <Button
                          size="sm"
                          variant={choice === "not-met" ? "default" : "outline"}
                          onClick={() => setDecisions({ ...decisions, [key]: "not-met" })}
                        >
                    ...
                    <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-secondary">
                      <div
                        className={`h-full ${met ? "bg-success" : "bg-destructive"}`}
          ...
          <SectionCard title="Review timeline">
            <ol className="space-y-2">
              ...
              {activeCase.extended === false && (
                <li className="rounded-lg border border-dashed border-border p-2.5 text-xs text-muted-foreground">
                  R5 / R6 added on extension
                </li>
              )}
            </ol>
          </SectionCard>
          ...
          {saved && (
            <div className="rounded-xl border border-success/30 bg-success/10 p-4 text-sm">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Document updated. Continue to confirmation.
              </p>
              <Button asChild size="sm" className="mt-3 w-full">
                <Link to="/supervisor/pip/document">
                  <FileText className="mr-2 h-3.5 w-3.5" /> View updated document
                </Link>
              </Button>
            </div>
          )}
