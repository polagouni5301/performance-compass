...
export default function Inbox() {
  ...
  const list = capCases.filter(
    (c) => c.status === "logged" || c.status === "disputed" || c.status === "accepted",
  );
              ...
              {open && (
                <div className="mt-4 rounded-xl border border-primary/30 bg-primary-soft/40 p-4 text-sm">
                  ...
                  <p className="mt-2 text-muted-foreground">
                    A discussion meeting will be booked on Microsoft Teams.
                  </p>
                  <ul className="mt-3 grid gap-1.5 text-xs">
                    <li className="flex items-center gap-2">
                      <Video className="h-3.5 w-3.5" /> Teams meeting · auto-scheduled this week
                    </li>
                    <li className="flex items-center gap-2">
                      <Mail className="h-3.5 w-3.5" /> Confirmation emailed to agent (CC manager)
                    </li>
                  </ul>
                </div>
              )}
