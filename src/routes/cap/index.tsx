...
export const Route = createFileRoute("/cap/")({
  head: () => ({
    meta: [
      { title: "CAP Dashboard — Compliance Action Plans" },
      { name: "description", content: "QA & Compliance breach logging, CAP recommendation, dispute and exception workflows." },
    ],
  }),
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline" asChild><Link to="/cap/cases"><FileSearch className="mr-2 h-4 w-4" /> All cases</Link></Button>
            <Button asChild><Link to="/cap/new"><Plus className="mr-2 h-4 w-4" /> Log breach</Link></Button>
          </>
        }
      />
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <StatCard label="Disputed" value={disputed} hint="Max 2 attempts allowed" icon={AlertTriangle} tone="warning" />
        <StatCard label="Exception pending" value={exceptions} hint="Awaiting Mgr/SDL approval" icon={ShieldAlert} tone="default" />
        <StatCard label="HR escalations" value={escalated} hint="CAP 3 cases" icon={AlertTriangle} tone="danger" />
      </div>

      <div className="mt-8 grid gap-6 lg:grid-cols-3">
        <div className="rounded-2xl border border-border bg-gradient-warm p-6 shadow-soft lg:col-span-2">
          <div className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">CAP Levels & validity</div>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <div className="rounded-xl border border-border bg-card/70 p-4">
              ...
              <p className="text-xs text-muted-foreground">Valid 90 days. First-time / minor breach.</p>
            </div>
            <div className="rounded-xl border border-border bg-card/70 p-4">
              <CAPLevelBadge level="CAP 2" />
              <div className="mt-2 text-sm font-semibold">Strong warning</div>
              <p className="text-xs text-muted-foreground">Valid 90 days. Repeat / serious breach.</p>
            </div>
            <div className="rounded-xl border border-border bg-card/70 p-4">
        ...
        <SectionCard title="Workflow">
          <ol className="space-y-3 text-sm">
            {["Log breach (QA/Compliance)", "Auto-recommend CAP level", "Notify supervisor + manager", "Accept or dispute (max 2)", "Optional exception path", "Generate CAP letter", "Agent acknowledgement", "Close & audit"].map((s, i) => (
              <li key={i} className="flex gap-3">
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-primary-soft text-xs font-bold text-primary">{i + 1}</span>
                {s}
              </li>
            ))}
          </ol>
      ...
      <SectionCard title="All CAP cases" className="mt-8" actions={<Button size="sm" variant="outline" asChild><Link to="/cap/cases">View all</Link></Button>}>
            ...
            <tbody className="divide-y divide-border">
              {capCases.map((c) => (
                <tr key={c.id} className="hover:bg-secondary/40">
                  ...
                  <td className="py-3 pr-3"><CAPLevelBadge level={c.level} /></td>
                  <td className="py-3 pr-3"><CAPStatusBadge status={c.status} /></td>
                  <td className="py-3 pr-3 text-right">
                    <Button size="sm" variant="ghost" asChild><Link to="/cap/cases/$caseId" params={{ caseId: c.id }}>Open</Link></Button>
                  </td>
                </tr>
              ))}
            </tbody>
