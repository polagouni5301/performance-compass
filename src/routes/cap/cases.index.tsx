...
export const Route = createFileRoute("/cap/cases/")({
  head: () => ({
    meta: [
      { title: "All CAP cases" },
      { name: "description", content: "Browse and filter every Compliance Action Plan case." },
    ],
  }),
  component: CAPList,
});

const filters = ["all", "logged", "accepted", "disputed", "exception-pending", "closed", "hr-escalation"] as const;

function CAPList() {
  const [filter, setFilter] = useState<(typeof filters)[number]>("all");
  const [team, setTeam] = useState<"all" | "QA" | "Compliance">("all");
  const [q, setQ] = useState("");

  const list = capCases.filter((c) => {
    if (filter !== "all" && c.status !== filter) return false;
    if (team !== "all" && c.raisedByTeam !== team) return false;
    if (q && !`${c.employee.name} ${c.employee.ohrId} ${c.id} ${c.breachType}`.toLowerCase().includes(q.toLowerCase())) return false;
    return true;
  });

  return (
    <div>
      <PageHeader eyebrow="CAP Module" title="All CAP cases" description="Full lifecycle traceability across QA & Compliance breaches." />

      <SectionCard
        title={`${list.length} case${list.length !== 1 ? "s" : ""}`}
        actions={
          <div className="flex gap-2">
            <select value={team} onChange={(e) => setTeam(e.target.value as any)} className="rounded-lg border border-input bg-background px-3 py-1.5 text-sm">
