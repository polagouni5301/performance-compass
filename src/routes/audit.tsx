...
export const Route = createFileRoute("/audit")({
  head: () => ({
    meta: [
      { title: "Audit Dashboard — Full lifecycle traceability" },
      { name: "description", content: "Audit-ready view of every PIP and CAP case with download and full lifecycle traceability." },
    ],
  }),
  component: Audit,
});
...
function Audit() {
  const [tab, setTab] = useState<"pip" | "cap">("pip");
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <StatCard label="PIP closed — success" value={closedSuccess} hint="Exit criteria met" icon={ShieldCheck} tone="success" />
                ...
                {pipCases.map((c) => {
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-secondary/40">
                      ...
                      <td className="py-3 pr-3 font-mono text-xs text-muted-foreground">{c.employee.ohrId}</td>
                      ...
                      <td className="py-3 pr-3 text-xs text-muted-foreground">{c.acknowledgedAt ?? "—"}</td>
                      <td className="py-3 pr-3"><PIPStatusBadge status={c.status} /></td>
                      <td className="py-3 pr-3 text-right">
                        <Button size="sm" variant="ghost" asChild><Link to="/pip/cases/$caseId" params={{ caseId: c.id }}>View</Link></Button>
                      </td>
                    </tr>
                  );
                })}
              ...
              <tbody className="divide-y divide-border">
                {capCases.map((c) => (
                  <tr key={c.id} className="hover:bg-secondary/40">
                    ...
                    <td className="py-3 pr-3 text-right">
                      <Button size="sm" variant="ghost" asChild><Link to="/cap/cases/$caseId" params={{ caseId: c.id }}>View</Link></Button>
                    </td>
                  </tr>
                ))}
              </tbody>
