                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {isProcessed ? (
                      <StatusBadge variant={isProcessed === "accepted" ? "success" : "danger"}>
                        {activeTab === "disputes"
                          ? isProcessed === "accepted"
                            ? "Dispute Accepted & Closed"
                            : "Dispute Rejected"
                          : isProcessed === "accepted"
                            ? "Exception Approved & Closed"
                            : "Rejected"}
                      </StatusBadge>
                    ) : activeTab === "disputes" ? (
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="rounded-xl bg-secondary/40 p-4 text-sm leading-relaxed border border-border/50 shadow-inner">
                        {c.supervisorComment ||
                          "Disagree with framing — context: peak workload + system outage at the time. Requesting reduced level."}
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {(c.documents || ["dispute_evidence_01.pdf"]).map((doc, idx) => (
                          <div
                            key={idx}
                            className="flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 text-xs font-medium hover:border-primary/40 transition-colors cursor-pointer group"
                          >
                    ...
                    {activeTab === "exceptions" && c.status === "exception-pending" && (
                      <div className="mt-2 border-t border-border pt-4">
                        ...
                        <p className="rounded-xl bg-info/5 p-4 text-sm leading-relaxed border border-info/20 text-info font-medium italic">
                          This exception request is currently awaiting approval from{" "}
                          <span className="font-semibold">{c.exceptionApprover}</span>. Once
                          reviewed, the manager/SDL will either approve or reject the request, and
                          their comments will be visible here.
                        </p>
                      </div>
                    )}
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder={
                          activeTab === "disputes"
                            ? "Log outcome of offline discussion…"
                            : "Internal notes for exception monitoring…"
                        }
                    ...
                    {activeTab === "exceptions" &&
                      (isProcessed === "rejected" || c.status === "exception-rejected") && (
                        <div className="mt-2 border-t border-border pt-4">
                          ...
                          <p className="rounded-xl bg-destructive/5 p-4 text-sm leading-relaxed border border-destructive/20 text-destructive font-medium italic">
                            {c.managerComment ||
                              "Rejection: The justification provided does not meet the criteria for a policy exception. Agent must adhere to the standard documentation process regardless of volume."}
                          </p>
                        </div>
                      )}
                      ...
                      {!isProcessed ? (
                        <div className="flex flex-row items-center gap-3 pt-4 border-t border-border">
                          <Button
                            ...
                            size="sm"
                            className={cn(
                              "text-xs border border-border shadow-sm hover:bg-secondary/80",
                              activeTab === "exceptions" ? "w-full" : "shrink-0",
                            )}
                            asChild
                          >
                        ...
                        <div className="rounded-xl border border-dashed border-border p-6 text-center space-y-2 bg-secondary/10">
                          ...
                          <p className="text-sm font-semibold">
                            Case {isProcessed === "accepted" ? "Approved" : "Rejected"}
                          </p>
                          <p className="text-xs text-muted-foreground">
                            The case has been marked as closed in the audit log.
                          </p>
                        </div>
                      )}
                    </div>
