...
export const Route = createFileRoute("/qa/disputes")({
  head: () => ({
    meta: [
      { title: "QA — Dispute Resolution" },
      { name: "description", content: "Issuer reviews supervisor disputes, logs discussion notes, and resolves or escalates as exception." },
    ],
  }),
  component: Disputes,
});
...
function Disputes() {
  ...
  return (
    <div>
      ...
      {list.length === 0 ? (
        <SectionCard><p className="text-sm text-muted-foreground">No active disputes.</p></SectionCard>
      ) : (
            ...
            >
              <div className="grid gap-6 lg:grid-cols-3">
                <div className="space-y-3 lg:col-span-2">
                  <div>
                    <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Supervisor response</div>
                  ...
                  <div>
                    ...
                    <textarea rows={3} placeholder="Log outcome of offline discussion…" className="mt-1.5 w-full rounded-xl border border-input bg-background px-3 py-2.5 text-sm focus:border-ring focus:outline-none" />
                ...
                <aside className="space-y-2">
                  <Button className="w-full"><Check className="mr-2 h-4 w-4" /> Accept dispute</Button>
                  <Button variant="outline" className="w-full"><X className="mr-2 h-4 w-4" /> Reject dispute</Button>
                  <Button variant="ghost" className="w-full" asChild>
                    <Link to="/cap/exceptions"><ShieldAlert className="mr-2 h-4 w-4" /> Raise exception</Link>
                  </Button>
                </aside>
              </div>
            </SectionCard>
