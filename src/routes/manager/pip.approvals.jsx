...
import { Mail, Check, X, FileText } from "lucide-react";
...
export default function Approvals() {
  ...
  return (
    <div>
      ...
      {pending.length === 0 ? (
        <SectionCard>
          <p className="text-sm text-muted-foreground">No extension requests pending.</p>
        </SectionCard>
      ) : (
                  ...
                  <div className="lg:col-span-2">
                    ...
                    <div className="mt-4 rounded-xl border border-warning/30 bg-warning/10 p-4 text-sm">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Only {met} of 4 reviews met (need 3). Recommendation: extend by 30 days with
                        2 additional reviews (R5 & R6).
                      </p>
                    </div>
                  </div>
                  ...
                  <aside className="space-y-3">
                    <div className="rounded-xl border border-border bg-secondary/40 p-4 text-sm">
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        Sent to <strong>{c.employee.manager}</strong> with full review summary
                        attached.
                      </p>
                      <Button size="sm" variant="outline" className="mt-3 w-full">
                        <Mail className="mr-1 h-3.5 w-3.5" /> View email thread
                      </Button>
                    </div>
                    ...
                    <Button variant="outline" className="w-full">
                      <X className="mr-2 h-4 w-4" /> Reject — refer to HR
                    </Button>
