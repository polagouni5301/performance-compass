...
export const Route = createFileRoute("/pip/candidates")({
  head: () => ({
    meta: [
      { title: "PIP Candidates — Trigger evaluation" },
      { name: "description", content: "Employees flagged by PIP trigger logic awaiting supervisor confirmation." },
    ],
  }),
    ...
    <div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {pipCandidates.map((cand) => (
          <article key={cand.employee.ohrId} className="rounded-2xl border border-border bg-card p-5 shadow-soft">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  {cand.employee.team} · {cand.employee.type === "apprentice" ? "Apprentice" : "Regular"}
                </div>
              </div>
        ...
        <ol className="grid gap-4 text-sm md:grid-cols-4">
          {[
            ...
            <li key={s.n} className="rounded-xl border border-border bg-secondary/40 p-4">
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary text-xs font-bold text-primary-foreground">{s.n}</div>
