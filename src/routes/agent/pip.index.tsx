...
export const Route = createFileRoute("/agent/pip/")({
  head: () => ({
    meta: [
      { title: "Agent — My PIP" },
      { name: "description", content: "Agent's PIP summary, review schedule, targets, and acknowledgement status." },
    ],
  }),
  component: MyPIP,
});
        ...
        <div className="space-y-6 lg:col-span-2">
          <SectionCard title="Targets">
            <ul className="space-y-2">
              {c.targets.map((t, i) => (
                <li key={i} className="rounded-xl border border-border bg-secondary/40 p-3 text-sm">{t}</li>
              ))}
            </ul>
          </SectionCard>
          ...
          <SectionCard title="Review schedule">
            <ol className="space-y-2">
              {c.reviews.map((r) => (
                <li key={r.number} className="flex items-center justify-between rounded-xl border border-border bg-card p-3 text-sm">
                  <div>
                    <div className="font-medium">Review {r.number}</div>
                    <div className="text-xs text-muted-foreground">{r.date} {r.kpiScore !== undefined && `· KPI ${r.kpiScore}`}</div>
                  </div>
            ...
            <div className="rounded-xl border border-border bg-secondary/40 p-4">
              <div className="flex items-center gap-3">
                ...
                <div className="text-xs">
                  <div className="font-semibold">PIP_{c.id}_v{c.documentVersion}.docx</div>
          ...
          <SectionCard title="Acknowledgement">
            {c.acknowledgedAt ? (
              <div className="text-sm">
                ...
                <div className="mt-1 font-mono text-xs text-muted-foreground">📧 {c.acknowledgmentEmail}</div>
              </div>
            ) : (
              <Button asChild className="w-full"><Link to="/agent/pip/acknowledge"><Mail className="mr-2 h-4 w-4" /> Acknowledge PIP</Link></Button>
            )}
          </SectionCard>
        </aside>
      </div>
