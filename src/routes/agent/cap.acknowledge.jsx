...
export default function AckCAP() {
  const [done, setDone] = useState(false);
  return (
    <div>
      <PageHeader
        eyebrow="Agent · CAP"
        title="Acknowledge CAP / Warning"
        description="Confirm you have read and understood the CAP letter."
      />
          ...
          <SectionCard title="Letter summary" actions={<CAPLevelBadge level="CAP 2" />}>
            <ul className="space-y-2 text-sm">
              <li className="flex justify-between">
                ...
                <span className="font-medium">CAP-2025-0455</span>
              </li>
              <li className="flex justify-between">
                <span className="text-muted-foreground">Breach</span>
                <span className="font-medium">Compliance — Disclosure</span>
              </li>
              <li className="flex justify-between">
                <span className="text-muted-foreground">Validity</span>
                <span className="font-medium">90 days from issue</span>
              </li>
            </ul>
            <p className="mt-3 rounded-lg bg-secondary/40 p-3 text-sm leading-relaxed">
              Failed to disclose conflict of interest within SLA. Repeat occurrence may escalate to
              CAP 3 (HR-managed).
            </p>
          </SectionCard>
        </div>
            ...
            {done ? (
              <div className="rounded-xl border border-success/30 bg-success/10 p-4 text-sm">
                ...
                <Button asChild size="sm" className="mt-3 w-full">
                  <Link to="/agent/cap">Back to CAPs</Link>
                </Button>
              </div>
            ) : (
