...
export const Route = createFileRoute("/agent/")({
  head: () => ({
    meta: [
      { title: "Agent — My cases" },
      { name: "description", content: "Agent view of received PIP and CAP cases." },
    ],
  }),
  component: AgentHome,
});
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
        <StatCard label="Active PIP" value={myPIP ? "Yes" : "No"} hint={myPIP?.id ?? "—"} icon={TrendingUp} tone="primary" />
        <StatCard label="Open CAPs" value={myCAPs.filter(c => c.status !== "closed").length} icon={ShieldAlert} tone="warning" />
        <StatCard label="Pending acknowledgements" value={(myPIP && !myPIP.acknowledgedAt ? 1 : 0) + myCAPs.filter(c => !c.acknowledgedAt).length} icon={Mail} />
      </div>

      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <SectionCard title="My PIP" actions={<Button size="sm" asChild><Link to="/agent/pip">Open</Link></Button>}>
          {myPIP ? (
            <div className="space-y-2 text-sm">
              ...
              <div className="text-xs text-muted-foreground">{myPIP.startDate} → {myPIP.endDate}</div>
              <div className="text-xs">Reviews met: <span className="font-semibold">{myPIP.reviews.filter(r => r.status === "met").length}/{myPIP.reviews.length}</span></div>
            </div>
          ) : <p className="text-sm text-muted-foreground">No active PIP.</p>}
        </SectionCard>

        <SectionCard title="My CAP / Warnings" actions={<Button size="sm" asChild><Link to="/agent/cap">Open</Link></Button>}>
          {myCAPs.length === 0 ? <p className="text-sm text-muted-foreground">No CAPs on record.</p> : (
