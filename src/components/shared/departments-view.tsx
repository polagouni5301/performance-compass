...
import { departments, channelLabel, type DepartmentChannel } from "@/lib/mock-data";
import { Briefcase, Users, TrendingUp, ShieldAlert } from "lucide-react";
import { useState } from "react";

const channelTone: Record<DepartmentChannel, "primary" | "info" | "warning"> = {
  voice: "primary",
  messaging: "info",
  cdt: "warning",
};
...
export function DepartmentsView({
  ...
  const [filter, setFilter] = useState<DepartmentChannel | "all">("all");
  ...
  const tabs: { id: DepartmentChannel | "all"; label: string }[] = [
    { id: "all", label: "All" },
  ...
  return (
    <div>
        ...
        {tabs.map((t) => (
          <button
            ...
            className={`rounded-lg px-3 py-1.5 text-sm font-semibold transition ${
              filter === t.id ? "bg-background text-foreground shadow-soft" : "text-muted-foreground hover:text-foreground"
            }`}
          >
            {t.label}
          </button>
      ...
      <SectionCard className="mt-4" title={`${list.length} department${list.length === 1 ? "" : "s"}`}>
            ...
            <tbody className="divide-y divide-border">
              {list.map((d) => (
                <tr key={d.name} className="hover:bg-secondary/40">
                  ...
                  <td className="py-3 pr-3"><StatusBadge variant={channelTone[d.channel]}>{channelLabel[d.channel]}</StatusBadge></td>
